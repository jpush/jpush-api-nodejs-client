...
function ios (alert, sound, badge, contentAvailable, extras, category, mutableAvailable) {
  if (alert != null) {
    if (typeof alert !== 'string') {
  ...
  if (mutableAvailable != null) {
    if (typeof mutableAvailable !== 'boolean') {
      throw new JError.InvalidArgumentError('Invalid ios.mutable-available, it can only be set to the boolean.')
    }
    ios['mutable-available'] = mutableAvailable
  }
  return {'ios': ios}
}
...
function setPeriodicalSchedule (start, end, time, timeUnit, frequency, point) {
  var periodical = {'start': start, 'end': end, 'time': time, 'time_unit': timeUnit,
