  ...
  if (args.length == 1 && typeof args[0] === 'string') {
    ...
    for (i = 0; i < args[0].length; i++) {
      if (typeof args[0][i] != 'string') {
        throw new JError.InvalidArgumentError('Invalid ' + title + ' at index ' + i + ', ' + title
          + ' can only be set to the String')
      }
    ...
    for (i = 0; i < args.length; i++) {
      if (typeof args[i] != 'string') {
        throw new JError.InvalidArgumentError('Invalid ' + title + ' at argument ' + i + ', ' + title
          + ' can only be set to the String')
      }
      payload.push(args[i])
    }
