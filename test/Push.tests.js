...
describe('Push test', function() {
    ...
    it('Push platform test1', function(done) {
            ...
            client.push().setPlatform('android', 'ios', 'winphone')
                    .setAudience(JPush.ALL).setNotification(Base.ALERT).send(
                            function(err, res) {
                                if (!err && res) {
                                    done();
                                }
    ...
    it('Push tags test', function(done) {
            ...
            client.push().setPlatform(JPush.ALL).setAudience(
                    JPush.tag(Base.TAG1)).setNotification(Base.ALERT).send(
                    function(err, res) {
                        if (!err && res) {
                            done();
                        }
    ...
    it('Push android full', function(done) {
            ...
            client.push().setPlatform(JPush.ALL).setAudience(JPush.ALL)
                    .setNotification(
                            JPush.android(Base.ALERT, Base.TITLE, 1,
    ...
    it('Push message test2', function(done) {
            ...
            client.push().setPlatform(JPush.ALL).setAudience(JPush.ALL)
                    .setMessage(Base.MSG_CONTENT, Base.TITLE, 'content type',
                            Base.EXTRAS).send(function(err, res) {
                        if (!err && res) {
                            done();
                        }
    ...
    it('Options test1', function(done) {
            ...
            client.push().setPlatform(JPush.ALL).setAudience(JPush.ALL)
                    .setNotification(Base.ALERT).setOptions(123456, 60, null,
                            true).send(function(err, res) {
                        if (!err && res) {
                            done();
                        }
                    });
